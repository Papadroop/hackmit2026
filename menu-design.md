...
Built to this document with the four changes recorded in §6.3, §7.1, §7.2 and below. Everything else is as specified. Nothing was committed; `contract/`, `backend/`, `fixtures/` and `demo-documents/` were not touched.
...
| Scroll probe, 20 steps over 2 s (§13) | ≥ 50 fps | 59.3 fps |
...
**Two things worth a second opinion, both as specified:**

- The title crosses from white to ink over p 0.46–0.54 as §6.4 gives. At the midpoint it is mid-grey against a background that is half clean paper and half pigment, about 2.0–2.5:1 for roughly 4 % of the wrapper's scroll. It stays readable at 316 px, and §6.4 invites those two numbers to be adjusted, so narrowing the window is a one-line change if it reads wrong on stage.
