...
- **Confidence** of a verdict is a judgment in the fixture, bounded by its dimensions: not above the highest dimension confidence, not more than 0.1 below the lowest. Step 17 defines the formula (evaluator agreement, judge consistency across runs, evidence tier); until then the validator only checks the bounds.
- **Document summary** numbers are not derived by the validator beyond consistency checks (distribution equals the verdict counts, counts equal the lists, ranks are 1..n, targets exist). Step 18 owns the aggregation.
...
- Deferred: the confidence formula (step 17), document aggregation (step 18), re-scoring semantics for live runs, the company view's cross-document objects (step 18).
